/**
 * GERTRAN v4 — ZERO zoom em lugar nenhum.
 * Tremor vinha do zoompan fazendo interpolação pixel-by-pixel em images 512→1080.
 * Agora: scale lanczos direto, imagem estática, micro fade-in entre cenas.
 */
...
const OUT_FILE = path.join(OUT_DIR, 'gertran-fisioterapeuta-30s-v4.mp4');
...
const TMP = '/tmp/gertran-v4-texts/';
...
// Inputs: padding (1 img × 2s) + main (N imgs × 1.25s) + music + narr
const inputs = [];
inputs.push('-framerate', String(FPS), '-loop', '1', '-t', String(PAD_DUR), '-i', PAD_IMG);
...
// PADDING — scale lanczos, SEM zoompan, SEM movimento
filters.push(
  `[0:v]scale=${SQ}:${SQ}:flags=lanczos,fade=t=in:st=0:d=0.2,` +
  `pad=${W}:${H}:0:420:color=0x0A1428,format=yuv420p,setsar=1[vpad]`
);

// MAIN — cada imagem scale lanczos + fade-in 0.1s. Sem zoom.
imgs.forEach((_, idx) => {
  const inputStream = idx + 1;
  filters.push(
    `[${inputStream}:v]scale=${SQ}:${SQ}:flags=lanczos,fade=t=in:st=0:d=0.1,` +
...
console.log(`[gertran v4] SEM zoom, scale lanczos, fade-in 0.1s entre cenas`);
const t0 = Date.now();
const res = spawnSync('ffmpeg', args, { stdio: ['ignore', 'inherit', 'inherit'] });
if (res.status !== 0) process.exit(res.status || 1);
const stat = fs.statSync(OUT_FILE);
console.log(`\n[gertran v4] ✅ ${OUT_FILE} — ${(stat.size/1024/1024).toFixed(1)} MB em ${((Date.now()-t0)/1000).toFixed(1)}s`);
