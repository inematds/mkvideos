/**
 * GERTRAN v7 — ajustes finais do piloto:
 *   - Layout v6a fixado (caption y=1520, logo abaixo do vídeo)
 *   - Nome da profissão GIGANTE ocupando quase a largura
 *   - Rodapé: INEMA.CLUB maiúsculo em negrito (substitui @inema.club)
 */
...
const OUT_FILE = path.join(OUT_DIR, 'gertran-fisioterapeuta-31s-v7.mp4');
...
const TMP = '/tmp/gertran-v7-texts/';
...
// PADDING hook (0-3s)
// Tag profissão GIGANTE — quase ocupando a largura
// Com Bebas Neue (condensed), fontsize 150 dá ~960px de largura pra "FISIOTERAPEUTA" (14 chars)
const hookPadding = [
  drawtext({ text: PROFESSION,         t0: 0, t1: PAD_DUR, y: 230,  size: 150, color: '#FFEB3B', bord: 8 }),
  drawtext({ text: 'VOCE JA CUROU',    t0: 0, t1: PAD_DUR, y: 690,  size: 118, bord: 10 }),
  drawtext({ text: 'DOR DE UM JEITO',  t0: 0, t1: PAD_DUR, y: 830,  size: 118, bord: 10 }),
  drawtext({ text: 'QUE NINGUEM',      t0: 0, t1: PAD_DUR, y: 970,  size: 118, bord: 10 }),
  drawtext({ text: 'ENSINA MAIS.',     t0: 0, t1: PAD_DUR, y: 1110, size: 118, color: '#FFEB3B', bord: 10 }),
];
...
console.log(`[gertran v7] profissão 150pt + rodapé INEMA.CLUB 80pt`);
const t0 = Date.now();
const res = spawnSync('ffmpeg', args, { stdio: ['ignore', 'inherit', 'inherit'] });
if (res.status !== 0) process.exit(res.status || 1);
const stat = fs.statSync(OUT_FILE);
console.log(`\n[gertran v7] ✅ ${OUT_FILE} — ${(stat.size/1024/1024).toFixed(1)} MB em ${((Date.now()-t0)/1000).toFixed(1)}s`);
